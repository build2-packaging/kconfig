...
static inline int is_absolute(const char *path)
{
	...
	return *path == '/' ? 1 : 0;
#else
	return path[0] != '\0' && path[1] == ':' ? (path[2] == '/' ? 3 : 2) : 0;
#endif
}
...
static bool is_same(const char *file1, const char *file2)
{
	...
	ret = true;
#else
	HANDLE map1_h = CreateFileMappingA((HANDLE)_get_osfhandle(fd1), NULL,
					   PAGE_READONLY, 0, st1.st_size, NULL);

	if (map1_h == NULL)
		goto close2;

	HANDLE map2_h = CreateFileMappingA((HANDLE)_get_osfhandle(fd2), NULL,
					   PAGE_READONLY, 0, st1.st_size, NULL);
...
int conf_read_simple(const char *name, int def)
{
	...
	if (name) {
		...
		env = conf_getenv("KCONFIG_DEFCONFIG_LIST");
		if (!env)
			return 1;
